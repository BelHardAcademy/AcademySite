...
$('.block6-item').each(function(i) {
	$(this).find('.student-name').text(arr[i].userName).end()
	       .find('.user-about').text(arr[i].userAbout).end()
	       .find('.body').text(arr[i].body).end()
	       .find('.block6-foto img').attr('src', arr[i].userPictLink);//apiImg + result[i].userPictLink
		});
	}
